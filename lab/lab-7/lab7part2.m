...
for k=1:numFrames
  ...
  rxTxt = dataHat(payloadStart:end); % Take the actual payload
  rxTxt = reshape(rxTxt, 7, []); % Condition data
  rxTxt = transpose(rxTxt); % More conditioning
  ...
  % Increment count of total frames.
  total = total + 1;
  
  % Check if frame correct.
  if(rxTxt == "Arizona")
    % Increment if frame correct.
    correct = correct + 1;
  end
end

%% Get percentage of correct frames.
